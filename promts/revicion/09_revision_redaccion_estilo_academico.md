...
9. **Uso de macros semánticas**: verificar que no se utilicen comandos manuales de bajo nivel cuando existan macros estandarizadas en `estilos.sty` y `estilos/caratula.sty` (`\capitulopreliminar`, `\begin{estilodedicatoria}{Título}`, `\imprimircaratulabth`, `\begin{estilocaratulabth}`, `\insertarmarcobth`, `\seccionanexo`, `\configurarseccionfinal`, `\palabrasclave`, `\notatabla`, etc.).
